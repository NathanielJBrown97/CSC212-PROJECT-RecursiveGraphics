...
/*void initiateSierpinski(int HEIGHT, int WIDTH, std::string fileName, sf::Color userColor) {
    ...
    graphic.setInvisibleBox(0.0, 0.0, WIDTH,
                            HEIGHT); // Snowflake Class setter - sets the dimensions of our invisible box in which the snowflake will be constructed; passed Width / Height from functions params.
    graphic.setColor(userColor); // Snowflake Class setter - sets the color.
    graphic.setStageOutput(
            stage); // Snowflake Class setter - sets the number of iterations the recursive graphic generation will undergo (i.e. sets the stage of the final output drawing); passed int variable representing number of iterations.
    ...
    while (graphic_window.isOpen()) {
        ...
        graphic.construct_SierpinskiTriangle(graphic_window); // Snowflake Class Method - calls the method used to start actually drawing our snowflake; passed the window object.
        ...
        requiredSolution.update(
                graphic_window); // Update the texture to be the current set of pixels within the graphic_window
...
void SierpinskiTriangle::construct_SierpinskiTriangle(sf::RenderWindow& graphic_window){
    ...
    {
        //end recursive calls
        constructATriangle(top, leftPoint, rightPoint, colorGraphic, graphic_window);
    }
    ...
    else
    {
        ...
        constructATriangle(top, leftPoint, rightPoint, colorGraphic, graphic_window); // creates our initial triangle passed top, left, right, color, and window object
...
void SierpinskiTriangle::recursive_koch_HELPER(sf::Vector2f startPoint, sf::Vector2f endPoint, int currentStage, sf::RenderWindow& graphic_window)
{
    /// BASE CASE
    ...
    if(currentStage == FinalStage)
    {
        ...
        sf::Vector2f topPoint = sf::Vector2f(0.5 * (startPoint.x - endPoint.x) + endPoint.x, 0.5 * (startPoint.y - endPoint.y) + endPoint.y); // top = 2d vector storing (x,y) || X = One third the difference of start pos x - end pos x, + end pos x. Y = the same as previous; substitute y for x.
        sf::Vector2f leftPoint = sf::Vector2f(0.5 * (startPoint.x - endPoint.x) + endPoint.x, 0.5 * (startPoint.y - endPoint.y) + endPoint.y); // left = same as above, but with two thirds substituted for one third.
        ...
        constructATriangle(topPoint, leftPoint, rightPoint, colorGraphic, graphic_window); //calls the draw for our final triangle passed these defined points
    ...
    else
    {
        ...
        sf::Vector2f nextTopPoint = sf::Vector2f(0.5 * (endPoint.x - startPoint.x) + startPoint.x, 0.5 * (endPoint.y - startPoint.y) + startPoint.y); // nextThird = 2d vector storing (x,y) || X = One third the difference of start pos x - end pos x, + end pos x. Y = the same as previous; substitute y for x.
        /*sf::Vector2f nextLeftPoint = sf::Vector2f(0.5 * (nextTopPoint.x - startPoint.x) + startPoint.x, sqrt(3)*(0.5 * (nextTopPoint.x - startPoint.x) + startPoint.x)); // nextTwoThird =  same as above, but with two thirds substituted for one third.
        sf::Vector2f nextRightPoint = sf::Vector2f(0.5 * (endPoint.x - nextTopPoint.x) + nextTopPoint.x, sqrt(3)*(0.5 * (nextTopPoint.x - startPoint.x) + startPoint.x)); // pass the nextThird and nextTwoThird points to our top point function to return a new top point of the two points.
*/
        ...
        constructATriangle(nextTopPoint, nextLeftPoint, nextRightPoint, sf::Color::Blue, graphic_window);
        ...
        recursive_koch_HELPER(startPoint, nextTopPoint, currentStage+1,  graphic_window); //passed active functions startCoords, nextThird value as our end, incremented iteration, and our window.
        recursive_koch_HELPER(nextTopPoint, endPoint, currentStage+1,  graphic_window); //passed twoThirds, active functions endCoords, incremented iteration, and our window.
        recursive_koch_HELPER(nextLeftPoint, nextTopPoint, currentStage+1,  graphic_window); //passed nextThird vlaue, nextTop value, incremented iteration, and our window.
        recursive_koch_HELPER(nextTopPoint, nextRightPoint, currentStage+1,  graphic_window); //passed nextTop value, twoThirds value, incremented iteration, and our window.
    }
}
...
void SierpinskiTriangle::setColor(const sf::Color& desiredColor)
{
